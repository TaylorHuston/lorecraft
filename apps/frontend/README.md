...
Playwright always starts isolated frontend and backend services on dedicated test ports. It requires `E2E_DATABASE_URL` plus `ALLOW_E2E_DATABASE_WRITES=1`, applies migrations to the disposable database, and exercises the backend only through the frontend's same-origin `/api` proxy.
