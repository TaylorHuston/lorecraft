...
| 2026-07-22 | Steep lower-story fade | `AdventureWorkbench` CSS and Storybook | Replaced the broad fade with a transparent lower dock through 56%, a slight 35% background blend at 67%, and full background at 78%. | commit candidate |
...
| Adventure ready composer | `Application/Adventures/Workbench` ReadyDesktop | 1440x900 desktop; 390x844 narrow | Storybook plus `agent-browser` | Lower narration stays clear for most of the composer dock, then has a brief steep fade before disappearing. Its 96px text area ends halfway through the joined, equal-width Send/Pass row; the controls straddle that lower border with no gap. | No Vite overlay; long-story simulation inspected; action widths are 72px, gap is 0px, and the row midpoint equals the input bottom. | passed |
