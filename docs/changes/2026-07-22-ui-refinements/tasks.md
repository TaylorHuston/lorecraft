...
- [x] Add a keyboard-operable read-only Player profile section to Adventure Settings.
...
| 2026-07-22 | Settings Player profile | `AdventurePage`, CSS, focused route test, Storybook | Added a read-only Player tab showing the existing Adventure profile and current state: name, physical description, backstory, status, and current location. The four-tab narrow layout uses two columns without horizontal overflow. | commit candidate |
...
| Adventure Settings workspace | `Application/Adventures/Workbench` ReadyToAct | 1440x900 desktop; 390x844 narrow | Storybook plus `agent-browser` | A wide modal shows Adventure Settings, a read-only Player profile, NPCs, and Locations in its left navigation, with a bounded Reset section. The narrow layout uses two tab columns without horizontal overflow. | No page errors; all tabs and Player profile fields present. | passed |
...
- LC-003/S1 R5 and S3 now distinguish player-visible Scene NPC details from Settings-only Debug cards, alongside Player-pane controls, a read-only Settings Player profile, and rendered Storybook proof. LC-003/S2 will distinguish the owner-visible Act/Pass/Guide transcript from the narration-only model context. No release communication is expected because this is a private local product-flow refinement.
