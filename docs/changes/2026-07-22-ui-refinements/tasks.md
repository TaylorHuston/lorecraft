...
| 2026-07-22 | Story paragraph rhythm and player-cue regression | focused workbench, Storybook, lint, and rendered browser | The transcript test asserts one visible “You” cue per Player message. The ReadyDesktop fixture renders a multi-paragraph narration with one line-height of separation; Player entries have a visible eyebrow and thin right accent with no card fill. | passed; 19 focused workbench tests, 20 focused Storybook tests, lint, direct browser inspection |
...
| Owner chat transcript | `Application/Adventures/Workbench` ReadyDesktop and TurnPending | 1280x640 desktop; ready Act/Pass/Guide state | Storybook plus `agent-browser` | Ready view shows four unboxed Game Master messages at left, Act/Pass/Guide messages at right, italicized Guide text, a visible “You” cue, slim player-side accents, and one line-height between an entry's paragraphs. | Meaningful content, no Vite overlay, empty captured console-error list. | passed |
...
- LC-003/S1 R5 and S3 now distinguish player-visible Scene NPC details from Settings-only Debug cards, alongside Player-pane controls and rendered Storybook proof. LC-003/S2 will distinguish the owner-visible Act/Pass/Guide transcript from the narration-only model context. No release communication is expected because this is a private local product-flow refinement.
