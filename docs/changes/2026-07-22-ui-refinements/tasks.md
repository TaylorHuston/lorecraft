...
| Composer prompt helper | `Application/Adventures/Workbench` ReadyDesktop | Desktop Act/Guide; 390x844 narrow Act | Storybook plus `agent-browser` | The italic dynamic prompt is inside the composer box above the placeholder; the input remains in its prior vertical position and Send/Pass stay clear. | No Vite overlay; DOM confirms the helper is inside, its computed font style is italic, and the accessible textbox label changes with the mode. | passed |
