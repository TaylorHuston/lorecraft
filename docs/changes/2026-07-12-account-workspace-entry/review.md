...
- [ ] `apps/frontend/src/app/App.tsx:10` disables focus revalidation while the server session expires after two hours. An already-rendered `/worlds` route can therefore continue displaying private account state after the session expires or is revoked elsewhere. Implement `LC-001/S3/R1-S3` session revalidation and prove the open workspace returns to sign-in after expiry or external revocation.
- [ ] `apps/backend/config/bodyparser.ts:50` globally auto-processes multipart uploads up to 20 MB before the route-level signup and login throttles execute. Repeated unauthenticated multipart requests can therefore consume temporary storage, bandwidth, CPU, and file descriptors before receiving `429`. Disable unused multipart auto-processing or restrict it to explicit upload routes, enforce a small pre-parser auth payload/content-type boundary, and add regression evidence that auth routes reject oversized multipart input before file processing.
