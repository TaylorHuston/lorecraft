...
| S1/R2-S1, S1/R2-S2, S1/R2-S3                                         | `apps/backend/tests/database/frozen_world_source_migration.spec.ts`, `apps/backend/tests/functional/world_version_publication.spec.ts`, `apps/backend/tests/functional/adventure_creation_service.spec.ts`, `apps/backend/tests/functional/adventure_query_service.spec.ts`, and `apps/frontend/e2e/starter-world.spec.ts` | Immutable source/default constraints, stable snapshot publication, initial binding, playability conflicts, and an existing Adventure retaining its frozen NPC while a newly created Adventure uses published canon. | Passing 2026-07-20 against a guarded isolated schema |
