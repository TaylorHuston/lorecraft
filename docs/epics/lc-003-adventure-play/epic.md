...
| S2/R1-S3, S2/R3-S4 | Automated test `apps/backend/tests/functional/adventure_turn_worker.spec.ts#LC-003/S2/R1-S3 + R3-S4: rejects even short reflected Guide text before narration publication` and unit tests `apps/backend/tests/unit/story_generation/adventure_turn_context.spec.ts#LC-003/S2/R1-S3: rejects even concise current Guide text before publication` and `apps/backend/tests/unit/story_generation/adventure_turn_context.spec.ts#LC-003/S2/R1-S3: permits ordinary narration for punctuation-only and common-token Guides` | Direct meaningful current-Guide reflection is rejected before extraction or publication, while punctuation-only and common-token Guidance does not reject ordinary narration. | Passing 2026-07-22 against a guarded disposable schema |
