...
| ----- | ----------- | ------------------------------------- | ------------- | --------------------------------------------------------------------------- |
| S1    | implemented | Start and resume a private Adventure. | 2026-07-18    | Opening, privacy, recovery, lifecycle, and responsive workbench foundation. |
...
| `apps/backend/app/services/adventure_opening_policy.ts`                                                                                                                                                                                         | Defines persistence-independent retry classification and bounded scheduling policy.                                                                             | Recheck when provider failures, attempt limits, backoff, or retry guidance change.                        |
| `apps/backend/database/migrations/1784323200000_minimize_model_call_evidence.ts`                                                                                                                                                                | Replaces prompt/raw-response evidence columns with bounded request/response metadata.                                                                           | Recheck when model-call evidence retention changes.                                                       |
| `apps/frontend/src/app/AppRoutes.tsx` and `apps/frontend/src/adventures/creationRequestId.ts`                                                                                                                                                   | Own route title/focus presentation and portable UUID creation identities.                                                                                       | Recheck when route hierarchy or creation identity changes.                                                |
| `apps/backend/Dockerfile`, `apps/frontend/Dockerfile`, `deploy/compose.yaml`, and `deploy/release-command.mjs`                                                                                                                                  | Package and coordinate the separately supervised API, worker, same-origin gateway, guarded migration, health, and rollback boundaries.                          | Recheck when production process commands, networking, release order, or rollback changes.                 |
| `apps/backend/app/controllers/health_checks_controller.ts` and `.github/workflows/images.yml`                                                                                                                                                   | Expose liveness/readiness and build immutable production images from `main`.                                                                                    | Recheck when health dependencies or image provenance changes.                                             |
...
| S1/R1-S4 and R5-S5..R5-S6 local UI walkthrough                            | Taylor's provider-backed Adventure creation and opening review                                                                                                                                                                                                                                                                                               | The processing notice, pending state, ready opening, Player context, Scene context, and overall Adventure presentation are understandable and accepted. Automated tests remain the evidence for exact title, heading-focus, and non-stealing live-region mechanics.                                                                   | User confirmed 2026-07-18                                         |
...
| S1/R5-S4                                                                  | `apps/frontend/src/adventures/AdventureWorkbench.test.tsx`, `apps/frontend/src/comparison/Workbench.stories.tsx`, and `apps/frontend/e2e/adventure-foundation.spec.ts`                                                                                                                                                                                       | Desktop Player/Story/Scene composition with Story-first document headings and unchanged visual grid, Story-first keyboard tabs, lifecycle states, touch access, and overflow-free desktop/mobile rendering.                                                                                                                           | Passing 2026-07-18                                                |
...
| S1/R1-S4 and R1-S5                                                        | `apps/frontend/src/adventures/AdventureRoutes.test.tsx` and `apps/frontend/src/adventures/creationRequestId.test.ts`                                                                                                                                                                                                                                         | The configured-provider notice precedes submission and the native/fallback request identity remains UUID v4 compliant.                                                                                                                                                                                                                | Passing 2026-07-18                                                |
| S1/R3-S6 and R3-S7                                                        | `apps/backend/tests/unit/story_generation/openai_compatible_story_generator.spec.ts` and `apps/backend/tests/unit/adventure_opening_policy.spec.ts`                                                                                                                                                                                                          | Provider-returned evidence contains bounded metadata only; raw bodies/headers are absent; byte counts avoid intermediate buffers; retry classification, jitter, terminal-status handling, extreme attempt inputs, and bounded delta/date `Retry-After` are deterministic.                                                               | Passing 2026-07-18                                                |
| S1/R3-S6..R3-S8                                                           | `apps/backend/tests/unit/story_generation/openai_compatible_story_generator.spec.ts`, `apps/backend/tests/unit/adventure_opening_policy.spec.ts`, `apps/backend/tests/unit/adventure_opening_worker_port.spec.ts`, `apps/backend/tests/functional/adventure_opening_worker.spec.ts`, and `apps/backend/tests/database/adventure_aggregate_migration.spec.ts` | Metadata-only evidence, retry classification/backoff, persistence-port orchestration, cancellation rescheduling, leases, stale-work handling, and atomic publication pass against disposable Neon.                                                                                                                                    | Passing 2026-07-18                                                |
| S1/R3-S9 and R3-S10 deployment support                                    | `deploy/container-contract.test.mjs`, `deploy/release-command.test.mjs`, and `scripts/image-workflow.test.mjs`                                                                                                                                                                                                                                               | Static production contracts cover separate API/worker commands, dependency-free container liveness, bounded readiness probes, five-second polling, loopback-only gateway, trusted private-HTTPS forwarding, tolerant production env/state handling, one-shot migration, immutable `main` images, and application or rollback recovery.       | Supporting automation passing 2026-07-18; live deployment pending |
| S1/R5-S5 and R5-S6                                                        | `apps/frontend/src/app/RoutePresentation.test.tsx` and `apps/frontend/src/adventures/AdventureRoutes.test.tsx`                                                                                                                                                                                                                                               | Adventure destinations receive titles/heading focus, background completion is announced, and existing focus is preserved.                                                                                                                                                                                                             | Passing 2026-07-18                                                |
| S1/R1, R2, and typed client boundary                                      | `apps/backend/tests/database/adventure_aggregate_migration.spec.ts` and `apps/frontend/src/adventures/tuyauAdventureApi.test.ts`                                                                                                                                                                                                                             | Aggregate constraints and generated-client runtime validation support the accepted creation/read boundary.                                                                                                                                                                                                                            | Passing 2026-07-18                                                |

#### Verification Gaps

- The local Adventure presentation, provider notice, and pending-to-ready opening are user confirmed. Private-production HTTPS, session, host-independence, and recovery acceptance remain Change-level gaps.
...
- `S1/R3-S9` and `S1/R3-S10` have image, Compose, health, polling, coordinated-release, and rollback support, but the product Scenarios remain unverified until the images are deployed, private provider reachability and listeners are inspected, shutdown/restart is exercised, and one real opening is published while the developer laptop runs no Lorecraft process.
