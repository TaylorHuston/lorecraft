...
| S1/R3-S9 and R3-S10 deployment support                                    | `deploy/container-contract.test.mjs`, `deploy/release-command.test.mjs`, `scripts/image-workflow.test.mjs`, and private-production acceptance recorded in the related Change                                                                                                                                                                                   | Static contracts plus the deployed immutable `main` images prove separate API/worker supervision, five-second polling, loopback/tailnet-only ingress, private provider reachability, coordinated restart, one real opening, and deterministic application rollback with no laptop Lorecraft listener. | Passing and user confirmed 2026-07-18 |
...
- The local and private-production Adventure presentation, provider notice, pending-to-ready opening, private HTTPS session, host independence, restart, and rollback behavior are user confirmed.
...
- None for `S1/R3-S9` or `S1/R3-S10`; the private deployment, provider path, listeners, restart, rollback, and real opening were verified on 2026-07-18.
