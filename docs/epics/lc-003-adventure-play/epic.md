...
| ----- | ----------- | ------------------------------------- | ------------- | ------------------------------------------------------- |
| S1    | implemented | Start and resume a private Adventure. | 2026-07-16    | Implemented by the Private Adventure Foundation Change. |
...
| `apps/frontend/src/adventures/AdventurePage.tsx`, `AdventureWorkbench.tsx`, and `NewAdventurePage.tsx`                                       | Presents creation, pending, failure, ready, retry, reset, and responsive Player/Story/Scene states without owning domain rules.                               | Recheck when Adventure navigation, disclosure, lifecycle presentation, or responsive composition changes. |
| `apps/frontend/src/workspace/WorkspacePage.tsx` and `apps/frontend/src/worlds/WorldDetailPage.tsx`                                           | Present direct creation, owner Adventure summaries, resume, and confirmed deletion from both cross-World catalog and World detail contexts.                   | Recheck when Adventure discovery, launch, or lifecycle-management placement changes.                       |
...
| S1/R2-S1 disclosure boundary                                              | `apps/backend/tests/functional/adventure_api.spec.ts` and `world_catalog.spec.ts`                                                                   | Player-facing World and Adventure projections omit raw snapshots, author identity, prompt evidence, and private Character knowledge.                                                                                                                              | Passing 2026-07-16 |
| S1/R4 and R5 client behavior                                              | `apps/frontend/src/adventures/*.test.tsx`, `apps/frontend/src/worlds/WorldRoutes.test.tsx`, and Adventure/World Storybook stories                   | Typed creation/lifecycle behavior, catalog and detail launch/resume/delete, pending polling, retry/reset conflicts, filtered responsive context, keyboard tabs, focus, and automated accessibility checks.                                                        | Passing 2026-07-16 |
| S1/R1, R3, R4, and R5 routed journey                                      | `apps/frontend/e2e/adventure-foundation.spec.ts` with `fake-story-provider.mjs`                                                                     | Desktop/mobile creation, populated pending state, reload-safe deterministic opening, resume with zero turns, same-Adventure reset, anonymous/cross-owner denial, responsive tabs, overflow checks, and cleanup deletion through production API/worker boundaries. | Passing 2026-07-16 |
| S1/R1-R5 full regression gates                                            | Backend 101-test suite, frontend 101-test suite, Storybook 64-test interaction/accessibility suite, build/typecheck/lint, and Playwright 7-test suite | Existing account/World behavior and the complete deterministic Adventure foundation remain green together.                                                                                                                                                       | Passing 2026-07-16 |

#### Verification Gaps

- The deterministic provider proves orchestration and persistence, not live-provider narrative quality; a configured-model playtest remains required before review closeout.
- Final responsive composition and lifecycle placement still require Taylor's manual UI confirmation.
