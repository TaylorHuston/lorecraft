...
| `apps/frontend/src/comparison/Workbench.stories.tsx` | Exposes deterministic desktop, mobile, World-navigation analogue, empty, and error comparison fixtures. | Recheck when the comparison protocol or Adventure composition changes. |
...
| S1/R1-R5 full regression gates                                            | `apps/backend/tests/functional/adventure_api.spec.ts`, `apps/frontend/src/adventures/AdventureRoutes.test.tsx`, `apps/frontend/e2e/adventure-foundation.spec.ts`, and root `package.json` scripts                  | Backend 102-test, frontend 101-test, Storybook 64-test, build/typecheck/lint, and Playwright 7-test gates keep the complete deterministic Adventure foundation green together.                                                                                 | Passing 2026-07-17 |
...
- The strengthened `S1/R5-S3` and new `S1/R5-S4` app-owned evidence is complete. User confirmation and cross-application UI Foundations comparison capture remain Change-level closeout work.
