...
| `apps/frontend/src/auth/PasswordField.tsx` and `apps/frontend/src/components/{TextField,IconButton,Button}/` | Provide app-owned labeled fields, independent password disclosure, and pending action behavior without changing credential semantics. | Recheck when account controls or disclosure behavior changes.     |
...
- `S1/R3-S3` still requires desktop/mobile browser confirmation outside Storybook.
...
| `apps/frontend/src/auth/PasswordField.tsx` and `apps/frontend/src/components/{TextField,IconButton,Button}/` | Provide app-owned labeled fields, password disclosure, and pending action behavior without changing credential semantics. | Recheck when account controls or disclosure behavior changes. |
...
- `S2/R3-S3` still requires desktop/mobile browser confirmation outside Storybook.
- Production HTTPS verification of the session cookie's `Secure` attribute is explicitly deferred until before production deployment.
